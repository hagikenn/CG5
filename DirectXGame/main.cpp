...
ID3D12Resource* CreateRenderTextureResource(ID3D12Device* device, uint32_t width, uint32_t height, DXGI_FORMAT clearFormat, const FLOAT* clearColor) {
	...
	resourceDesc.Dimension = D3D12_RESOURCE_DIMENSION_TEXTURE2D;  // Textureの種類。普段使っているのは 2次元
	...
	HRESULT hr = device->CreateCommittedResource(
	    ...
	    &clearValue,                                // Clear最適値
	    IID_PPV_ARGS(&resource)                     // 生成するResourceポインタへのポインタ
	);
	assert(SUCCEEDED(hr));
...
ID3D12Resource* CreateDepthStencilTextureResource(ID3D12Device* device, int32_t width, int32_t height) {
	...
	resourceDesc.MipLevels = 1;                                   // mipmapの数 DepthStencilなので 1でいい
	resourceDesc.DepthOrArraySize = 1;                            // Textureの配列数 DepthStencilは 1でいい
...
int WINAPI WinMain(_In_ HINSTANCE, _In_opt_ HINSTANCE, _In_ LPSTR, _In_ int) {
	...
	// DirectXCommonクラスが管理している、ウインドウの幅と高さを取得
	...
	rtvDescriptorHeapDesc.NumDescriptors = 1;                    // Descripiorの個数は 1
	...
	// SRV (Shader Resource View) を準備する ※ PixelShaderと連携をとろうとするため ☆00_09 追加

	// 1. SRV 用の DescriptorHeapの作成
	...
	while (true) {
		...
		//SRVのDescripterTableの先頭を設定　※0はrootParameter[0]である　★00_09 追加
...
void SetupPipelineState(PipelineState& pipelineState, RootSignature& rs, Shader& vs, Shader& ps) {
	...
	// 書き込むRTVの情報
	graphicsPipelineStateDesc.NumRenderTargets = 1; // 1つのRTVに書き込む ※2つ同時に行うと思えばできる
